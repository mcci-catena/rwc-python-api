##############################################################################
# 
# Module: rwc5020x_test_1_30.py
#
# Description:
#     Unit test cases for sw version 1.305
#
# Copyright notice:
...
class RwcApiTest(unittest.TestCase):
    ...
    def test_rfchannelgroup_as923(self):
        self.assertEqual(self.rwctest.protocol_setregion('AS_923'), 'ACK', 'Set Region Failed')
        self.assertEqual(self.rwctest.rf_setchannelgroup_as923('AS_923-1'), 'ACK', 'Set RF Channel group AS_923 failed')
        self.assertEqual(self.rwctest.rf_getchannelgroup_as923(), 'AS_923-1', 'Get RF Channel group AS_923 failed')
        self.assertEqual(self.rwctest.rf_setchannelgroup_as923('AS_923-2'), 'ACK', 'Set RF Channel group AS_923 failed')
        self.assertEqual(self.rwctest.rf_getchannelgroup_as923(), 'AS_923-2', 'Get RF Channel group AS_923 failed')
        self.assertEqual(self.rwctest.rf_setchannelgroup_as923('AS_923-3'), 'ACK', 'Set RF Channel group AS_923 failed')
        self.assertEqual(self.rwctest.rf_getchannelgroup_as923(), 'AS_923-3', 'Get RF Channel group AS_923 failed')

    def test_rffreqoffset_as923(self):
        self.assertEqual(self.rwctest.protocol_setregion('AS_923'), 'ACK', 'Set Region Failed')
        self.assertEqual(self.rwctest.rf_setfreqoffset_as923(1), 'ACK', 'Set frequency offset AS_923 failed')
        self.assertEqual(self.rwctest.rf_getfreqoffset_as923(), '1.0000', 'Get frequency offset AS_923 failed')

    def test_rfchplan_cn470(self):
        self.assertEqual(self.rwctest.protocol_setregion('CN_470'), 'ACK', 'Set Region Failed')
        self.assertEqual(self.rwctest.rf_setchplan_cn470('20M_A'), 'ACK', 'Set ch plan for CN470 failed')
        self.assertEqual(self.rwctest.rf_getchplan_cn470(), '20M_A', 'Get ch plan for CN470 failed')
        self.assertEqual(self.rwctest.rf_setchplan_cn470('20M_B'), 'ACK', 'Set ch plan for CN470 failed')
        self.assertEqual(self.rwctest.rf_getchplan_cn470(), '20M_B', 'Get ch plan for CN470 failed')
        self.assertEqual(self.rwctest.rf_setchplan_cn470('26M_A'), 'ACK', 'Set ch plan for CN470 failed')
    ...
    def test_protocolregion(self):
        regionlist = ['EU_868', 'EU_433', 'US_915', 'AU_921', 'CN_470', 'KR_922', 'AS_923', 'IN_866']
    ...
    def test_protocolprotocolver(self):
        self.assertEqual(self.rwctest.protocol_setprotocolver('LoRaWAN1.0.4'), 'ACK', 'Set Operation Failed')
        self.assertEqual(self.rwctest.protocol_getprotocolver(), 'LoRaWAN1.0.4', 'Failed to read')
    ...
    def test_linkmalfunction(self):
        self.assertEqual(self.rwctest.link_setmalfunction('ON'), 'ACK', 'Set malfunction activation type failed')
        self.assertEqual(self.rwctest.link_getmalfunction(), 'ON', 'Failed to read')
        self.assertEqual(self.rwctest.link_setmalfunction('OFF'), 'ACK', 'Set malfunction activation type failed')
        self.assertEqual(self.rwctest.link_getmalfunction(), 'OFF', 'Failed to read')

    def test_linkmicerror(self):
        self.assertEqual(self.rwctest.link_setmicerror('ON'), 'ACK', 'Set mic err activation type failed')
        self.assertEqual(self.rwctest.link_getmicerror(), 'ON', 'Failed to read')
        self.assertEqual(self.rwctest.link_setmicerror('OFF'), 'ACK', 'Set mic err activation type failed')
        self.assertEqual(self.rwctest.link_getmicerror(), 'OFF', 'Failed to read')

    def test_linkmhdrerror(self):
        self.assertEqual(self.rwctest.link_setmhdrerror('ON'), 'ACK', 'Set MAC header err activation type failed')
        self.assertEqual(self.rwctest.link_getmhdrerror(), 'ON', 'Failed to read')
        self.assertEqual(self.rwctest.link_setmhdrerror('OFF'), 'ACK', 'Set MAC header err activation type failed')
        self.assertEqual(self.rwctest.link_getmhdrerror(), 'OFF', 'Failed to read')

    def test_linkxormhdr(self):
        self.assertEqual(self.rwctest.link_setxormhdr(0x01), 'ACK', 'Set XOR value for MAC header failed')
        self.assertEqual(self.rwctest.link_getxormhdr(), '0x01', 'Failed to read')

    def test_linkfhdrerror(self):
        self.assertEqual(self.rwctest.link_setfhdrerror('ON'), 'ACK', 'Set frame header err activation type failed')
        self.assertEqual(self.rwctest.link_getfhdrerror(), 'ON', 'Failed to read')
        self.assertEqual(self.rwctest.link_setfhdrerror('OFF'), 'ACK', 'Set frame header err activation type failed')
        self.assertEqual(self.rwctest.link_getfhdrerror(), 'OFF', 'Failed to read')

    def test_linkxorfhdr(self):
        self.assertEqual(self.rwctest.link_setxorfhdr(0x01), 'ACK', 'Set XOR value for Frame header failed')
        self.assertEqual(self.rwctest.link_getxorfhdr(), '0x00000000000001', 'Failed to read')

    def test_linkfragment(self):
        self.assertEqual(self.rwctest.link_setfragmentindex(1), 'ACK', 'Set fragment index failed')
        self.assertEqual(self.rwctest.link_setfragmentsize(1), 'ACK', 'Set fragment size failed')
        self.assertEqual(self.rwctest.link_setnumberoffragment(1), 'ACK', 'Set number of fragment failed')
        self.assertEqual(self.rwctest.link_setfragemntpadding(1), 'ACK', 'Set fragment padding failed')
        self.assertEqual(self.rwctest.link_setfragmentdescriptor(0x01), 'ACK', 'Set fragment descriptor failed')
        self.assertEqual(self.rwctest.link_setfragmentalgorithm(1), 'ACK', 'Set fragment algorithm failed')

    def test_linkmulticast(self):
        self.assertEqual(self.rwctest.link_setmulticastkey(0x01), 'ACK', 'Set multicast key failed')
        self.assertEqual(self.rwctest.link_setmulticastgroupid(1), 'ACK', 'Set multicast group id failed')
        self.assertEqual(self.rwctest.link_setmulticastaddress(0x01), 'ACK', 'Set multicast address failed')
        self.assertEqual(self.rwctest.link_setmulticastfrequency(865), 'ACK', 'Set multicast frequency failed')
        self.assertEqual(self.rwctest.link_setmulticastdr('DR0_SF12BW125'), 'ACK', 'Set multicast data rate failed')
        self.assertEqual(self.rwctest.link_setmulticastoption(1), 'ACK', 'Set multicast option failed')
        self.assertEqual(self.rwctest.link_setmulticastinterval(1), 'ACK', 'Set multicast interval for packets failed')

    def test_linkfirmwarereboot(self):
        self.assertEqual(self.rwctest.link_setfirmwarereboottimemode('TIME'), 'ACK', 'Set FM reboot time mode failed')
        self.assertEqual(self.rwctest.link_setfirmwarereboottimemode('ASAP'), 'ACK', 'Set FM reboot time mode failed')
        self.assertEqual(self.rwctest.link_setfirmwarereboottimemode('CANCEL'), 'ACK', 'Set FM reboot time mode failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootyear(2020), 'ACK', 'Set FM reboot year failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootmonth(1), 'ACK', 'Set FM reboot month failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootday(1), 'ACK', 'Set FM reboot day failed')
        self.assertEqual(self.rwctest.link_setfirmwarereboothour(1), 'ACK', 'Set FM reboot hour failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootminute(10), 'ACK', 'Set FM reboot minute failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootsecond(1), 'ACK', 'Set FM reboot second failed')
        self.assertEqual(self.rwctest.link_setfirmwarerebootcountdownval(0x01), 'ACK', 'Set FM reboot count down value failed')
        self.assertEqual(self.rwctest.link_setnextfirmwareversion(0x01), 'ACK', 'Set next firmware version failed')
        self.assertEqual(self.rwctest.link_deletefirmwareversion(0x01), 'ACK', 'Delete firmware version failed')

    def test_linkapptime(self):
        self.assertEqual(self.rwctest.link_setapptimeperiod(1), 'ACK', 'Set app layer time request period failed')
        self.assertEqual(self.rwctest.link_setapptimetransnumber(1), 'ACK', 'Set no. of transfers for time sync app layer failed')
    ...
    def test_powerscenario(self):
        ...
        self.assertEqual(self.rwctest.power_setscenario('CERTI_DL_CNT'), 'ACK', 'Set Operation Failed')
        self.assertEqual(self.rwctest.power_getscenario(), 'CERTI_DL_CNT', 'Failed to read')
    ...
    def test_sensitivitydownlinkslot(self):
        ...
        self.assertEqual(self.rwctest.protocol_setclass('C'), 'ACK', 'Set Class Failed')
        self.assertEqual(self.rwctest.sensitivity_setdownlinkslot('RXC'), 'ACK', 'Set Operation Failed')
        self.assertEqual(self.rwctest.sensitivity_getdownlinkslot(), 'RXC', 'Failed to read')
    ...
    def test_nstrx_preamblesize(self):
        self.assertEqual(self.rwctest.nst_rx_setpreamblesize(2), 'ACK', 'Set NST Rx preamble size failed')
        self.assertEqual(self.rwctest.nst_rx_getpreamblesize(), '2', 'Failed to read')
    ...
    def test_sysip(self):
        self.assertEqual(self.rwctest.sys_setiptype('STATIC'), 'ACK', 'Set IP type Failed.')
        self.assertEqual(self.rwctest.sys_getiptype(), 'STATIC', 'Read IP type Failed.')
        self.assertEqual(self.rwctest.sys_setiptype('DYNAMIC'), 'ACK', 'Set IP type Failed.')
        self.assertEqual(self.rwctest.sys_getiptype(), 'DYNAMIC', 'Read IP type Failed.')
